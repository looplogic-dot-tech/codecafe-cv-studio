...
# --- workspace.ts ---------------------------------------------------------
# Keep updatedAt stable unless CV/settings actually changed. This prevents
# phantom revisions caused by merely opening/reconnecting the application.
...
# Stable semantic signature: ignores navigation and modification timestamps.
...
# Revision digest must ignore savedAt.
...
# Remote-preserving helper: cloud saves can never erase CVs/libraries that already
# exist remotely when the current browser has an incomplete workspace.
...
# Autosave: establish semantic baseline, do nothing for startup/reconnect/navigation,
# and merge remote content before any real save.
...
# Explicit Sync gets the same remote-preservation rule.
...
# Loading from Drive must MERGE the Drive workspace with current local data.
...
# --- migration helper ------------------------------------------------------
# DOM-injected migration cannot directly update React state. Reload after a successful
# merge so the just-saved workspace (including Professional Library) becomes active.
...
print('v1.5.8 stability patch applied: semantic saves only, remote-preserving sync, persistent libraries, Drive merge-load.')
