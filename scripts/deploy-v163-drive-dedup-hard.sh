...
# Materialize the current application source.
...
# Remove the obsolete direct PDF button. The working print editor is the only PDF entry point.
s = re.sub(
    r'\s*<button\s+className="primary"\s+onClick=\{\(\)\s*=>\s*window\.print\(\)\}>\{t\.pdf\}</button>',
    '', s, count=1,
)
...
# Successful Drive load/connect clears stale error styling.
...
# Keep the ALREADY WORKING editor and margin engine untouched.
# Only move its launcher into the old blue PDF button slot.
p=Path('src/printEditorV3.ts')
pe=p.read_text(encoding='utf-8')
pe=pe.replace('const anchor = document.querySelector<HTMLElement>(".previewTop");',
              'const anchor = document.querySelector<HTMLElement>(".topActions");')
pe=pe.replace('button.className = "zoom printEditorLauncherV2";',
              'button.className = "primary printEditorLauncherV2";')
p.write_text(pe,encoding='utf-8')

# Runtime guard against any legacy injector recreating an extra Drive disconnect.
...
assert 'document.querySelector<HTMLElement>(".topActions")' in pe, 'print editor launcher is not in topActions'
assert 'button.className = "primary printEditorLauncherV2"' in pe, 'print editor launcher is not blue primary'
assert 'printEditorContentViewport' in pe, 'working margin editor marker missing'
print('PASS: Drive UI clean; legacy PDF button removed; working print editor preserved in top-right slot.')
PY
...
echo "============================================================"
echo "CODECAFE CV STUDIO 1.6.3 UI REPAIR DEPLOYED"
echo "✓ Sync UI preserved"
echo "✓ Google Drive = Load + ONE Disconnect"
echo "✓ old Download PDF button removed"
echo "✓ working Print / PDF editor moved to old blue top-right slot"
echo "✓ margin engine untouched and validated"
