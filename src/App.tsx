...
// Producción empieza vacía. Los ejemplos nunca deben convertirse en datos del usuario.
const seed: CV = structuredClone(blankCV);
...
const copy = {
  es: {
    ...
    description: "Descripción y resultado", repository: "Repositorio", repositoryHint: "URL de GitHub, GitLab u otro repositorio (opcional)", addProject: "＋ Añadir proyecto",
  ...
  en: {
    ...
    description: "Description and outcome", repository: "Repository", repositoryHint: "GitHub, GitLab or another repository URL (optional)", addProject: "＋ Add project",
...
// Las líneas heredadas, por ejemplo ": AWS · Azure · Docker", siguen siendo válidas.
...
function printableRepository(value: string | undefined, label: string): string {
  ...
  const safe = escapeHtml(repository);
  const safeLabel = escapeHtml(label);
  return /^https?:\/\//i.test(repository) ? `<p class="repository"><strong>${safeLabel}:</strong> <a href="${safe}">${safe}</a></p>` : `<p class="repository"><strong>${safeLabel}:</strong> ${safe}</p>`;
}

function printableBoldInline(value: string): string {
  const match = value.match(/^\*\*(.+?)\*\*$/);
  return match ? `<strong>${printableInlineText(match[1])}</strong>` : printableInlineText(value);
}
...
function buildPrintableHtml(cv: CV, lang: Lang, labels: (typeof copy)[Lang]): string {
  ...
  const jobs = cv.jobs.filter((job) => job.role || job.company || job.bullets).map((job) => `
    <div class="entry"><h3>${escapeHtml(job.role)} — ${printableBoldInline(job.company)}</h3><time>${escapeHtml(job.dates)}</time>
    <ul>${job.bullets.split("\n").filter(Boolean).map((line) => `<li>${escapeHtml(line)}</li>`).join("")}</ul></div>`).join("");
  const projects = cv.projects.filter((project) => project.name || project.description || project.repository).map((project) => `
    <div class="entry"><h3>${escapeHtml(project.name)}</h3><strong>${escapeHtml(project.stack)}</strong>${printableStructuredLines(project.description)}${printableRepository(project.repository, labels.repository)}</div>`).join("");
  const custom = cv.customSections.filter((item) => item.title || item.content)
    .map((item) => section(item.title || labels.customSection, printableStructuredLines(item.content))).join("");
  ...
  ${section(title("certifications", labels.certificationsHeading), printableStructuredLines(cv.certifications))}${custom}
  ${section(title("skills", labels.skillsHeading), printableStructuredLines(cv.skills))}
  ${section(title("education", labels.educationHeading), printableStructuredLines(cv.education))}
  ${section(title("languages", labels.languagesHeading), printableStructuredLines(cv.languages))}
  </body></html>`;
}
...
export default function Home() {
  ...
  const [ec2Loaded, setEc2Loaded] = useState(false);
  const [serverRevision, setServerRevision] = useState(0);
  const serverRevisionRef = useRef(0);
  const ec2SaveQueueRef = useRef<Promise<void>>(Promise.resolve());
  // Una sesión HttpOnly puede sobrevivir a cerrar la pestaña. Este indicador evita
  // descargar la misma biblioteca más de una vez durante el arranque.
  const restoredServerLibraryRef = useRef(false);
  ...
  const insertImportedBlocks = (blocks: ImportedBlock[]) => {
    const append = (existing: string | undefined, added: string) => [existing?.trim(), added.trim()].filter(Boolean).join("\n\n");
    setCV((current) => {
      const next = structuredClone(current);
      for (const block of blocks) {
        if (block.target === "custom") {
          next.customSections.push({ title: block.customTitle?.trim() || block.heading, content: block.text });
        } else if (block.target === "experience") {
          next.jobs.push({ role: block.heading, company: "", dates: "", bullets: block.text });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      ec2SaveQueueRef.current = ec2SaveQueueRef.current.then(async () => {
        ...
        if (googleToken && driveLoaded) await saveGoogleBackup(googleToken, document, googlePrintable());
  ...
  }, [cv, driveLoaded, ec2Loaded, googleToken, lang, photoOn, serverSession, template, workspace, workspaceReady]);
  ...
  const syncCloud = async () => {
    ...
    try {
      ...
      if (googleToken && driveLoaded) await saveGoogleBackup(googleToken, backupDocument(), googlePrintable());
      setCloudStatus("synced");
    } catch (error) {
  ...
  const mergeStoredBackup = async (backup: NonNullable<Awaited<ReturnType<typeof loadServerBackup>>>, preserved: CVWorkspace) => {
    const payload = isEncryptedEnvelope(backup.payload)
      ? await decryptBackup<BackupDocument>(backup.payload, syncPassword)
      : backup.payload as BackupDocument;
    if (payload.schema !== 2 || !isWorkspace(payload.workspace)) return preserved;
    const merged = mergeWorkspaces(preserved, payload.workspace);
    setWorkspace(merged);
    saveWorkspaceLocal(merged);
    const current = activeDocument(merged);
    setCV({ ...seed, ...current.cv, projects: current.cv.projects ?? [], customSections: current.cv.customSections ?? [] });
    setLang(current.settings.lang);
    setTemplate(current.settings.template);
    setPhotoOn(current.settings.photoOn);
    return merged;
  };
  const refreshEc2Library = async (preserved: CVWorkspace) => {
    const backup = await loadServerBackup();
    const merged = backup ? await mergeStoredBackup(backup, preserved) : preserved;
    setEc2Loaded(true);
    return merged;
  };
  const connectEc2 = async () => {
    ...
    try {
      ...
      setServerHistory(await listServerBackups());
      await refreshEc2Library(workspaceWithCurrent());
  ...
  const connectDrive = async () => {
    ...
    try {
      ...
      // Autorizar es insuficiente: el siguiente paso siempre incorpora lo que
      // ya existe en Drive a la biblioteca visible de esta instalación.
      await refreshDriveLibrary(token, workspaceWithCurrent());
  ...
  const refreshDriveLibrary = async (token: string, preserved: CVWorkspace) => {
    ...
    if (backup?.schema === 2 && isWorkspace(backup.workspace)) {
      ...
      setWorkspace(merged);
      saveWorkspaceLocal(merged);
      // La biblioteca sí se había fusionado, pero el editor continuaba mostrando
      // el estado anterior. Eso podía hacer parecer que la reautorización no cargó
      // nada y, peor, dejaba una edición posterior basada en el CV equivocado.
      const current = activeDocument(merged);
      setCV({ ...seed, ...current.cv, projects: current.cv.projects ?? [], customSections: current.cv.customSections ?? [] });
      setLang(current.settings.lang);
      setTemplate(current.settings.template);
  ...
  const usableDriveToken = async () => {
    if (googleToken) return googleToken;
    if (!cloudConfig.googleClientId || !hasKnownGoogleGrant()) return "";
    const token = await authorizeGoogleDrive(cloudConfig.googleClientId);
    setGoogleToken(token);
    return token;
  };
  // La cookie de EC2 recupera la autorización, pero no contiene los CVs. Cargar
  // únicamente el estado de sesión dejaba la pantalla local vacía tras volver a
  // abrir CV Studio. Una vez lista la copia local, traemos y fusionamos el respaldo.
  useEffect(() => {
    if (!workspaceReady || restoredServerLibraryRef.current) return;
    restoredServerLibraryRef.current = true;
    let cancelled = false;
  ...
  // La operación debe hacerse una vez con el espacio de trabajo ya inicializado.
  ...
  const openLibrary = async () => {
    ...
    try {
      ...
      if (serverSession) combined = await refreshEc2Library(combined);
      const token = await usableDriveToken();
      if (token) await refreshDriveLibrary(token, combined);
    } catch (error) {
  ...
  const exportTxt = () => {
    const sections = [
      ...
      cv.projects.some((p) => p.name || p.description || p.repository) && `${sectionTitle("projects", t.projectsHeading).toUpperCase()}\n${cv.projects.map((p) => `${p.name} | ${p.stack}\n${p.description}${p.repository ? `\n${t.repository}: ${p.repository}` : ""}`).join("\n\n")}`,
    ...
    <main lang={lang} spellCheck={true}>
      <header className="topbar">
        ...
        <div className="topActions">
          ...
          <button className={`topActionButton cloudButton ${cloudStatus}`} onClick={openCloud} title={cloudStatusText}>☁ <span>{t.cloud}</span></button>
      ...
      <section className="workspace">
        <aside className="editor">
          ...
          {tab === "it" && <div className="stack">
            ...
            {cv.projects.map((project, i) => <div className="jobCard" key={i}>
              ...
              <Field label={t.repository}><input className={inputClass} type="url" placeholder="https://github.com/..." value={project.repository ?? ""} onChange={(e) => setProject(i, "repository", e.target.value)} /><small className="hint">{t.repositoryHint}</small></Field>
            </div>)}
            <button className="add" onClick={() => setCV((v) => ({ ...v, projects: [...v.projects, { name: "", stack: "", description: "", repository: "" }] }))}>{t.addProject}</button>
        ...
        <section className="previewPane">
          ...
          <article className={`paper ${template}`}>
            ...
            {cv.certifications && <CVSection title={sectionTitle("certifications", t.certificationsHeading)} defaultTitle={t.certificationsHeading} onTitleChange={(value) => setSectionTitle("certifications", value)} resetLabel={t.resetTitle}><StructuredLines value={cv.certifications} /></CVSection>}
            {cv.customSections.filter((section) => section.title || section.content).map((section, index) => <CVSection title={section.title || t.customSection} key={`${section.title}-${index}`}><StructuredLines value={section.content} /></CVSection>)}
...
function BoldInlineText({ value }: { value: string }) {
  const match = value.match(/^\*\*(.+?)\*\*$/);
  return match ? <b><InlineText value={match[1]} /></b> : <InlineText value={value} />;
}
